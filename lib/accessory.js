...
class BaseAccessory {
  constructor(log, config, accessoryType) {
    this.log = log;
    this.config = config || {};
    ...
    switch (this.accessoryType) {
      case AccessoryType.Hygrothermograph:
    ...
    this.log.debug(`Initialized accessory of type ${this.accessoryType}`);
  }
  ...
  setupMQTTClient() {
    ...
    let client;
    if (this.accessoryType === AccessoryType.Hygrothermograph) {
      const {
        temperatureTopic,
        humidityTopic,
        batteryTopic,
    ...
    } else if (this.accessoryType === AccessoryType.MiFlora) {
      const {
        temperatureTopic,
        illuminanceTopic,
        moistureTopic,
  ...
  getFertilityService() {
    const fertilityService = new Service.LightSensor(this.fertilityName);
    fertilityService
      .getCharacteristic(Characteristic.CurrentAmbientLightLevel)
  ...
  getServices() {
    ...
    switch (this.accessoryType) {
      ...
      default:
        throw Error(
          `getServices not implemented for Accessory "${this.accessoryType}"`
        );
    }
...
class MiFloraAccessory extends BaseAccessory {
  constructor(log, config) {
    super(log, config, AccessoryType.MiFlora);
  }
}

class HygrothermographAccessory extends BaseAccessory {
  constructor(log, config) {
    super(log, config, AccessoryType.Hygrothermograph);
  }
}
...
module.exports = (homebridge) => {
  ...
  return { HygrothermographAccessory, MiFloraAccessory };
};
